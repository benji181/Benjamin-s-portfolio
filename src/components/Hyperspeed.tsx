...
const Hyperspeed = ({
    ...
    const speedUpValue = useRef(0);
    const speedUpTarget = useRef(0);
    const lastUpdateTime = useRef(0);

    // Smooth interpolation function with easing
    const smoothLerp = (current: number, target: number, smoothing: number, deltaTime: number): number => {
        const exponential = Math.exp(-smoothing * deltaTime);
        return current + (target - current) * (1 - exponential);
    };
    ...
    useEffect(() => {
        ...
        class App {
            ...
            update(delta) {
                // Smooth interpolation for speed changes
                const smoothing = 8.0; // Higher = smoother, slower transition
                this.speedUp = smoothLerp(this.speedUp, speedUpTarget.current, smoothing, delta);
                ...
                const fovChange = smoothLerp(this.camera.fov, this.fovTarget, smoothing * 0.5, delta);
                if (Math.abs(fovChange - this.camera.fov) > 0.001) {
    ...
    `;
        ...
        // Smooth mouse event handlers
        const handleMouseDown = () => {
            isMouseDown.current = true;
            speedUpTarget.current = effectOptions.speedUp;
            if (effectOptions.onSpeedUp) effectOptions.onSpeedUp();
            myApp.fovTarget = effectOptions.fovSpeedUp;
        };

        const handleMouseUp = () => {
            isMouseDown.current = false;
            speedUpTarget.current = 0;
            if (effectOptions.onSlowDown) effectOptions.onSlowDown();
            myApp.fovTarget = effectOptions.fov;
        };

        // Add smooth event listeners to the window
